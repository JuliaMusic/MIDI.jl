...
getnotes(midi, args...)
```
which obtains all note-specific information and stores it as a vector of notes, which we call `Notes`. For example, this piece of code
```julia
using MIDI
...
# Extensions
This easy to use high level interface allows **MIDI.jl** to be extendable. For example, in another software package **MusicManipulations.jl** we provide general functions for manipulating (and futher analyzing) music data.
